...
h.append(q);x.append("<h4 class='control-sidebar-heading'>Skins</h4>");x.append(h);w.append(x);d("#control-sidebar-home-tab").after(w);l();function v(y){d("body").toggleClass(y);
c.layout.fixSidebar();if(y=="layout-boxed"){c.controlSidebar._fix(d(".control-sidebar-bg"));}if(d("body").hasClass("fixed")&&y=="fixed"){c.pushMenu.expandOnHover();
c.controlSidebar._fixForFixed(d(".control-sidebar"));c.layout.activate();}}function e(y){d.each(n,function(z){d("body").removeClass(n[z]);});d("body").addClass(y);
f("skin",y);return false;}function f(y,z){if(typeof(Storage)!=="undefined"){localStorage.setItem(y,z);}else{alert("Please use a modern browser to properly view this template!");
}}function u(y){if(typeof(Storage)!=="undefined"){return localStorage.getItem(y);}else{alert("Please use a modern browser to properly view this template!");
}}function l(){var y=u("skin");if(y&&d.inArray(y,n)){e(y);}d("[data-skin]").on("click",function(z){z.preventDefault();e(d(this).data("skin"));});d("[data-layout]").on("click",function(){v(d(this).data("layout"));
});d("[data-controlsidebar]").on("click",function(){v(d(this).data("controlsidebar"));var z=!c.options.controlSidebarOptions.slide;c.options.controlSidebarOptions.slide=z;
if(!z){d(".control-sidebar").removeClass("control-sidebar-open");}});d("[data-sidebarskin='toggle']").on("click",function(){var z=d(".control-sidebar");
if(z.hasClass("control-sidebar-dark")){z.removeClass("control-sidebar-dark");z.addClass("control-sidebar-light");}else{z.removeClass("control-sidebar-light");
z.addClass("control-sidebar-dark");}});d("[data-enable='expandOnHover']").on("click",function(){d(this).attr("disabled",true);c.pushMenu.expandOnHover();
if(!d("body").hasClass("sidebar-collapse")){d("[data-layout='sidebar-collapse']").click();}});if(d("body").hasClass("fixed")){d("[data-layout='fixed']").attr("checked","checked");
}if(d("body").hasClass("layout-boxed")){d("[data-layout='layout-boxed']").attr("checked","checked");}if(d("body").hasClass("sidebar-collapse")){d("[data-layout='sidebar-collapse']").attr("checked","checked");
}}})(jQuery,$.AdminLTE);