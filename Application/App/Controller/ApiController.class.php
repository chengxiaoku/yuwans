...
class ApiController extends Controller
{
    ...
    public function getGameDeskListWithStatus(){
    	...
    	if (empty($userId)){
    		...
    		if (empty($keywords)){
    			$model = M('war t1');
    			$list = $model->field(" t1.*,t2.nickname real_name,t3.thumb,t3.title gamename,t4.title ")->join(" left join yw_user t2 on t1.user_id = t2.id ")->join(" left join yw_games t3 on t3.id = t1.game_id ")->join("left join yw_user_netbar t4 on t4.user_id = t1.netbar_id")->where(" t1.status = '$gameStatus' ")->order(' t1.begin_time desc ') ->page($pageNo,$pageSize)->select();

    		}else{
    			$model = M('war t1');
    			$list = $model->field(" t1.*,t2.nickname real_name,t3.thumb,t3.title gamename,t4.title ")->join(" left join yw_user t2 on t1.user_id = t2.id ")->join(" left join yw_games t3 on t3.id = t1.game_id ")->join("left join yw_user_netbar t4 on t4.user_id = t1.netbar_id")->where(" t1.status = '$gameStatus' and (t2.real_name like '%$keywords%' or t3.title like '%$keywords%') ")->order(' t1.begin_time desc') ->page($pageNo,$pageSize)->select();
...
//     				}
    			...
    			$this->responseJson(array('list'=>$data));
    		}else{
    			$this->responseJson();
    		}
    	}else{
    		...
    		if ($list){
    			...
    			$this->responseJson(array('isUserJoin'=>$userModel,'list'=>$data));
    		}else{
    			$this->responseJson();
    		}
    	}
...
// 		}
		if ($model->where(" id = $gameDeskId ")->save(array('win'=>$teamUser['role'],'status'=>'judge'))===false){
			$this->responseFailMessage('修改失败');
		}else{
	...
	public function isAuthExists(){
		...
		if (empty($auth)){
			...
			if ($data){
				$this->responseJson(array('phone'=>$data['tel']));
			}else{
...
// 				$info['userType'] = $userType;
	 ...
	 * 取得圈子广告
	 */
	public function getADListForSpace(){
		//页号
		$pageNo = $this->getVar('pageNo');
		//每页行数
		$pageSize = $this->getVar('pageSize');
		...
		//城市名带市
		$city = $this->getVar('city');
		$data = M('region')->field("region_id")->where("region_name='$city'")->select();
		$city_id = $data[0]['region_id'];
		$data_city = M('slider')->where("city_id=$city_id")->limit($start,$pageSize)->select();
		$img_arr = array();
		$title = array();
		$time_arr= array();
		$con = array();
		foreach($data_city as $val ){
			$img_arr[] = unserialize(base64_decode($val['images']));
			$title[] = $val['title'];
			$time_arr[] = $val['add_time'];
			$con[] = $val['content'];
		}
		//dump($img_arr);
		$data_mess = array(
	...
	public function register1()
	{
		...
		if(!$this->is_null_val($phone) || !$this->is_null_val($pwd)){
			$this->responseFailMessage('手机号或密码不能为空!');
			exit;
		}
		//检测 手机号码是否被注册
		$bool = D('user')->new_is_phone($phone);
		if(!$bool){
			$this->responseFailMessage('手机号码已存在!');
			exit;
		}else{
			//判断用户类型
			if($userType == 'netbar'){
				...
				if($add_id = D('user')->add($data)){
					if(M('user_netbar')->add($data)){
						$this->responseJson(array('userId'=>$add_id));
					}else{
						D('user')->delete($add_id);
						$this->responseFailMessage('注册失败');
					}
				}else{
					$this->responseFailMessage('注册失败');
	 ...
	 * 检测变量是否为空 为NULL
	 */
	private function is_null_val($var_mess){
		$var_mess = trim($var_mess);
		if(!empty($var_mess)){
			if(!is_null($var_mess)){
				return true;
			}
	...
	public function test(){
		$user = M('user t1');
		//$data = $user->field('t1.id,t2.user_id')->join(' yw_user_netbar t2 ON t1.id = t2.user_id')->select();
		$this->charset();

		$data = $this->memth($user,'data');
		dump($data);
	 ...
	 * 调试信息 (utf-8)
	 */
	private function charset($str = 'utf-8'){
	 ...
	 * 设置缓存
	 */
	private function memth($db,$data){
		$data = S($data);
		if(is_null($data) || !$data){
			$_data = $db->field('t1.id,t2.user_id')->join(' yw_user_netbar t2 ON t1.id = t2.user_id')->select();
			S('data',$_data,6000);
			return 1;
